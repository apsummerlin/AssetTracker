...
bool XModem::send_bulk_data(struct bulk_data container) {
  ...
  for(size_t j = 0; result && j < container.count; ++j) {
    for(size_t i = 0; i < _id_bytes; ++i) 
      blk_id[i] = container.id_arr[j*_id_bytes + i];
...
bool XModem::send(char * file) {
  byte send_data[1024];

  fileToSend = fatfs.open(file, O_READ);
  ...
  if (fileToSend) {
    ...
    do {
      ...
      bytesRead = fileToSend.read(send_data, 1024);
      if(!send(send_data, bytesRead, id))
      {
        break;
      }
      id++;
